...
GAME_TITLE	:= Bart Bash DS
GAME_SUBTITLE	:= Bart Bast Port
GAME_AUTHOR	:= YourLocalProgrammer
GAME_ICON	:= build/../icon.gif
...
SOUNDBANKINFODIR	:= $(BUILDDIR)/nitrofiles/music
ifeq ($(strip $(NITROFSDIR)),)
    SOUNDBANKDIR	:= $(BUILDDIR)/nitrofiles/music
else
    SOUNDBANKDIR	:= $(BUILDDIR)/nitrofiles/music
endif

AUDIODIRS	:= nitrofiles/audio
# Tools
# -----
